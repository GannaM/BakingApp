...
/**
 * This test demos a user opening StepDetail Activity on a Smartphone and pressing the Next button.
 * Button Previous must be disabled when the Activity is first launched.
 *
 * This is to test activity on Smartphone only, and will fail on a Tablet, because Tablet's view doesn't have buttons.
 */

@RunWith(AndroidJUnit4.class)
public class Smartphone_StepDetailActivityBasicTest {

    private final String stepJsonExtra = "{\"id\":0,\"description\":\"Recipe Introduction\",\"shortDescription\":\"Recipe Introduction\",\"thumbnailURL\":\"\",\"videoURL\":\"https://d17h27t6h515a5.cloudfront.net/topher/2017/April/58ffd974_-intro-creampie/-intro-creampie.mp4\"}";
    ...
    public void setup() {
        ...
        intent.putExtra(StepDetailActivity.RECIPE_EXTRA, recipeJsonExtra);
    ...
    public void clickNextButton() {
        ...
        onView(withId(R.id.button_previous)).check(matches(not(isEnabled())));

        onView(withId(R.id.button_next)).perform(click());

        onView(withId(R.id.button_previous)).check(matches(isEnabled()));
